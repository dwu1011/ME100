...
while True:
    ...
    if alt_flag == 1:
        error_old = target_alt-altitude # Calculates old error for differential term
        alt_flag = 0
    else:
        error_old = error    
    error= target_alt-altitude # Calculate e
    alt_integration_term += error*dt # Calculate integration
    alt_differential_term = (error - error_old)/dt   #Calculate the derivative
    throttle = K_P*error + K_I*alt_integration_term + K_D*alt_differential_term # PID Control
    if throttle < 0:
        throttle = 0
    elif throttle > 1:
        throttle = 1       
